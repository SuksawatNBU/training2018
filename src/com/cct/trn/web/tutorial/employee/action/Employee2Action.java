...
public class Employee2Action extends CommonAction  implements ModelDriven<EmployeeModel>, InterfaceAction{

	private static final long serialVersionUID = 1886407726572516676L;
	private EmployeeModel model = new EmployeeModel();
	
	//Constructor
	public Employee2Action() {
		try {
	...
	public String search() throws Exception {
		...
		try {
			...
			//2.
			result = ReturnType.SEARCH.getResult(); /*manageSearch(conn, model, model.getCriteria(), getPF_CODE().getSearchFunction());*/
	...
	public String searchPopup() throws Exception {
		System.out.println("searchPopup : EmployeeAction");
		String result = null;
		CCTConnection conn = null;
		try {
			//1.สร้าง connection โดยจะต้องระบุ lookup ที่ใช้ด้วย
			...
			//3.การค้นหา(ของแต่ละระบบ) ตัวอย่าง.ค้นหาข้อมูลผู้ใช้
			EmployeeManager manager = new EmployeeManager(conn, getUser(), getLocale());
			List<EmployeeSearch> listResult = manager.searchPopup(getModel().getCriteria());
